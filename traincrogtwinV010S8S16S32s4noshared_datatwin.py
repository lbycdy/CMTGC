...
import wandb
from utils.dataset_twinmask import OCIDVLGTWINDataset

from model.modelV010S8s16s32s4noshared_twinmaskr1 import CROG_TWIN
...
def get_parser():
    ...
    parser.add_argument('--config',
                        default='/home/lbycdy/work/crog_lb/config/OCID-VLG/20240830swincrogV010S8S16S32S4noshared_datatwinr1.yaml',
                        type=str,
                        help='config file')
...
def train_with_grasp(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        # forward
        with amp.autocast():
            pred, target, loss, loss_dict = model(image, image1, text, ins_mask,mask_all, grasp_qua_mask, grasp_sin_mask,
...
def validate_with_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        pred, target = model(image, image1, text, ins_mask,mask_all, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask,
                             grasp_wid_mask)
...
def main_worker(gpu, args):
    ...
    train_data = OCIDVLGTWINDataset(root_dir=args.root_path,
                            input_size=args.input_size,
                            word_length=args.word_len,
                            split='train',
                            version=args.version,
                            twin=args.use_twin_clip,args=args)
    val_data = OCIDVLGTWINDataset(root_dir=args.root_path,
                            input_size=args.input_size,
                            word_length=args.word_len,
                            split='val',
                            version=args.version,
    ...
    train_loader = data.DataLoader(train_data,
                                   ...
                                   drop_last=True,
                                   collate_fn=OCIDVLGTWINDataset.collate_fn)
