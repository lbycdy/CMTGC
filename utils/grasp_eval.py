...
# ... existing code ...
from utils.box_utils import crop, box_iou
...
def visualization(img, mask, target, grasp_masks, grasp_targets_masks, grasps, grasp_tar):
    grasp_qua_mask, grasp_ang_mask, grasp_wid_mask = grasp_masks
    grasp_qua_mask_label, grasp_ang_mask_label, grasp_wid_mask_label = grasp_targets_masks
    img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    fig = plt.figure(figsize=(25, 10))
    
    # draw grasp rectangles in image
    tmp = img_bgr.copy()
    tmp_label = img_bgr.copy()

    for rect, rect_label in zip(grasps, grasp_tar):
        center_x, center_y, width, height, theta = rect
        center_x_label, center_y_label, width_label, height_label, theta = rect_label
        box = ((center_x, center_y), (width, height), -(theta+180))
        box = cv2.boxPoints(box)
        box = np.intp(box)
        ptA, ptB, ptC, ptD = [list(map(int, pt.tolist())) for pt in box]
        tmp = cv2.line(tmp, ptA, ptB, (0,0xff,0), 2)
        tmp = cv2.line(tmp, ptD, ptC, (0,0xff,0), 2)
        tmp = cv2.line(tmp, ptB, ptC, (0,0xff,0), 2)
        tmp = cv2.line(tmp, ptA, ptD, (0,0xff,0), 2)



        box_label = ((center_x_label, center_y_label), (width_label, height_label), -(theta+180))
        box_label = cv2.boxPoints(box_label)
        box_label = np.intp(box_label)
        ptA_label, ptB_label, ptC_label, ptD_label = [list(map(int, pt_label.tolist())) for pt_label in box_label]
        tmp = cv2.line(tmp, ptA_label, ptB_label, (0, 0, 0xff), 2)
        tmp = cv2.line(tmp, ptD_label, ptC_label, (0, 0, 0xff), 2)
        tmp = cv2.line(tmp, ptB_label, ptC_label, (0, 0, 0xff), 2)
        tmp = cv2.line(tmp, ptA_label, ptD_label, (0, 0, 0xff), 2)
    tmp = cv2.resize(tmp, (520, 390))
    cv2.imshow('pre-grasps and label-grasps', tmp)
    ...
    msk_img[mask, 0] = 255
    mask = cv2.resize(mask,(520, 390))
    cv2.imshow("mask_pre1", mask)
    # indices = np.where(mask == 1)


    target = target.astype(np.uint8) * 255
    target = cv2.resize(target, (520, 390))
    cv2.imshow("mask_label", target)


    grasp_qua_mask = (grasp_qua_mask * 255).astype(np.uint8)
    heat_img = cv2.applyColorMap(grasp_qua_mask, cv2.COLORMAP_JET)
    grasp_qua_mask = cv2.addWeighted(img, 0.3, heat_img, 0.7, 0)
    grasp_qua_mask = cv2.resize(grasp_qua_mask, (520, 390))
    cv2.imshow('Grasp quality', grasp_qua_mask)

    grasp_qua_mask_label = (grasp_qua_mask_label * 255).astype(np.uint8)
    heat_img = cv2.applyColorMap(grasp_qua_mask_label, cv2.COLORMAP_JET)
    grasp_qua_mask_label = cv2.addWeighted(img, 0.3, heat_img, 0.7, 0)
    grasp_qua_mask_label = cv2.resize(grasp_qua_mask_label, (520, 390))
    cv2.imshow('Grasp quality label', grasp_qua_mask_label)

    grasp_ang_mask = (grasp_ang_mask * 255).astype(np.uint8)
    heat_img = cv2.applyColorMap(grasp_ang_mask, cv2.COLORMAP_JET)
    grasp_ang_mask = cv2.addWeighted(img, 0.3, heat_img, 0.7, 0)
    grasp_ang_mask = cv2.resize(grasp_ang_mask, (520, 390))
    cv2.imshow('Grasp Angle', grasp_ang_mask)

    grasp_ang_mask_label = (grasp_ang_mask_label * 255).astype(np.uint8)
    heat_img = cv2.applyColorMap(grasp_ang_mask_label, cv2.COLORMAP_JET)
    grasp_ang_mask_label = cv2.addWeighted(img, 0.3, heat_img, 0.7, 0)
    grasp_ang_mask_label = cv2.resize(grasp_ang_mask_label, (520, 390))
    cv2.imshow('Grasp Angle label', grasp_ang_mask_label)

    grasp_wid_mask = (grasp_wid_mask * 255).astype(np.uint8)
    heat_img = cv2.applyColorMap(grasp_wid_mask, cv2.COLORMAP_JET)
    grasp_wid_mask = cv2.addWeighted(img, 0.3, heat_img, 0.7, 0)
    grasp_wid_mask = cv2.resize(grasp_wid_mask, (520, 390))
    cv2.imshow('Grasp Width', grasp_wid_mask)

    grasp_wid_mask_label = (grasp_wid_mask_label * 255).astype(np.uint8)
    heat_img = cv2.applyColorMap(grasp_wid_mask_label, cv2.COLORMAP_JET)
    grasp_wid_mask_label = cv2.addWeighted(img, 0.3, heat_img, 0.7, 0)
    grasp_wid_mask_label = cv2.resize(grasp_wid_mask_label, (520, 390))
    cv2.imshow('Grasp Width label', grasp_wid_mask_label)
