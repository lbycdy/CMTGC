...
class OCIDVLGTWINDataset(Dataset):
    ...
    def preprocess(self, data):
        ...
        ins_mask = cv2.warpAffine(ins_mask,
                                  ...
                                  borderValue=0.)
        mask_all = cv2.warpAffine(mask_all,
                                  mat,
                                  self.input_size,
                                  flags=cv2.INTER_LINEAR,
                                  borderValue=0.)
    ...
    def collate_fn(batch):
        ...
        return {
            ...
            "img_numpy": [x["img_numpy"] for x in batch],
...
if __name__ == '__main__':
    ...
    for data in dataloader:
        ...
        mask_all = data['mask_all'].numpy().squeeze()
        ...
        cv2.imshow('Image with Mask_all', mask_all)
        cv2.imwrite('./images/Original Image.jpg', img)
        cv2.imwrite('./images/Mask.jpg', mask.astype(np.uint8) * 255)  # 掩码转换为可显示的图像
        cv2.imwrite('./images/Image with Mask.jpg', img_with_mask)
        cv2.imwrite('./images/Image with Mask_all.jpg', mask_all)
