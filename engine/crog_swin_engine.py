...
def train_with_grasp(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        # forward
        with amp.autocast():
            pred, target, loss, loss_dict = model(image,image1, text, ins_mask,mask_all, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, img_numpy)
...
def validate_with_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        pred, target = model(image,image1, text, ins_mask,mask_all, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, img_numpy)
...
def validate_without_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        # inference & get predictions from model
        pred, ins_mask_targets = model(image,image1, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask)
...
def inference_with_grasp(test_loader, model, args):
    ...
    for cnt, data in enumerate(tbar):
        ...
        # inference & get predictions from model
        pred, target = model(image, image1, text, ins_mask,mask_all, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask,image_numpy)
