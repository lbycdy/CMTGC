...
def inference_with_grasp(test_loader, model, args):
    ...
    # Load your custom image and word vector
...
def main():
    # Load your model
    model_data = torch.load('exp/OCID-VLG_multiple/crog_multiple_r50/last_model.pth', weights_only=True)
    if isinstance(model_data, dict):
        from model.crog import CROG  # Replace with your actual model class
        args = get_parser() # Load or define your configuration here
        model_instance = Clip() 

        for key in ["input_resolution", "context_length", "vocab_size"]:
            if key in model_data:
                del model_data[key]
        convert_weights(model_instance)
        model_instance.load_state_dict(model_data, False)
        model_instance = model_instance.cuda()
        model_instance.eval()
    else:
        model_instance = model_data.cuda()
        model_instance.eval()
    ...
    # Assuming you have a function to get the word vector
    word_vector = get_word_vector('grasp the right ball').unsqueeze(0).cuda()

    # Perform inference
    test_loader = [(image_tensor, word_vector)]  # Create a dummy data loader
    results = inference_with_grasp(test_loader, model_instance, args)
