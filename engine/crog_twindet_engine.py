...
from utils.dataset_twin import tokenize
...
def train_with_grasp(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        image = data["img"]
        image1 = data["img_twin"]
        image_det = data["img_det"]
        text = data["word_vec"]
        ins_mask = data["mask"]
        ...
        image_det = image_det.cuda(non_blocking=True)
        ...
        # import numpy as np
        #
        # original_image = image[0].detach().cpu().numpy()  # 转换为 numpy 数组
        ...
        # forward
        with amp.autocast():
            pred, target, loss, loss_dict = model(image,image1, image_det, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask)
...
def validate_with_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        pred, target = model(image,image1,image_det, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask)
...
def validate_without_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        # inference & get predictions from model
        pred, ins_mask_targets = model(image,image1,image_det, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, target)
...
def inference_with_grasp(test_loader, model, args):
    ...
    for cnt, data in enumerate(tbar):
        ...
        # inference & get predictions from model
        pred, target = model(image, image1, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask, target)
        ...
        grasp_qua_mask_preds = torch.sigmoid(grasp_qua_mask_preds)
        grasp_wid_mask_preds = torch.sigmoid(grasp_wid_mask_preds)
        grasp_sin_mask_preds = torch.sigmoid(grasp_sin_mask_preds)
        grasp_cos_mask_preds = torch.sigmoid(grasp_cos_mask_preds)


        # ins_mask_targets = torch.sigmoid(ins_mask_targets)
        # grasp_qua_mask_targets = torch.sigmoid(grasp_qua_mask_targets)
        grasp_wid_mask_targets= torch.sigmoid(grasp_wid_mask_targets)
        # grasp_sin_mask_targets = torch.sigmoid(grasp_sin_mask_targets)
        # grasp_cos_mask_targets = torch.sigmoid(grasp_cos_mask_targets)
        ...
        if ins_mask_preds.shape[-2:] != image.shape[-2:]:
            ...
            ins_mask_targets = F.interpolate(ins_mask_targets,
                                           size=image.shape[-2:],
                                           mode='bicubic',
                                           align_corners=True).squeeze(1)

            grasp_qua_mask_targets = F.interpolate(grasp_qua_mask_targets,
                                                 size=image.shape[-2:],
                                                 mode='bicubic',
                                                 align_corners=True).squeeze(1)

            grasp_sin_mask_targets = F.interpolate(grasp_sin_mask_targets,
                                                 size=image.shape[-2:],
                                                 mode='bicubic',
                                                 align_corners=True).squeeze(1)

            grasp_cos_mask_targets = F.interpolate(grasp_cos_mask_targets,
                                                 size=image.shape[-2:],
                                                 mode='bicubic',
                                                 align_corners=True).squeeze(1)

            grasp_wid_mask_targets = F.interpolate(grasp_wid_mask_targets,
                                                 size=image.shape[-2:],
                                                 mode='bicubic',
                                                 align_corners=True).squeeze(1)
        ...
        for idx in range(ins_mask_preds.shape[0]):
            ...
            ins_mask_target = inverse(ins_mask_target, inv_mat, w, h)
            ins_mask_target = (ins_mask_target > 0.35)
            grasp_qua_mask_target = inverse(grasp_qua_mask_target, inv_mat, w, h)
            grasp_sin_mask_target = inverse(grasp_sin_mask_target, inv_mat, w, h)
            grasp_cos_mask_target = inverse(grasp_cos_mask_target, inv_mat, w, h)
            grasp_wid_mask_target = inverse(grasp_wid_mask_target, inv_mat, w, h)
            ...
            for i in range(len(num_grasps)):
                ...
                grasp_target1, grasp_ang_mask_target1= detect_grasps(grasp_qua_mask_target, grasp_sin_mask_target,
                                                                 grasp_cos_mask_target, grasp_wid_mask_target, num_g)
                ...
                if args.visualize:
                    ...
                    visualization(img, ins_mask_pred, ins_mask_target, (grasp_qua_mask_pred, grasp_ang_mask_pred, grasp_wid_mask_pred),(grasp_qua_mask_target, grasp_ang_mask_target1, grasp_wid_mask_target), grasp_preds, grasp_target1)
