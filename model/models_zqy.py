...
@BACKBONES.register_module()
class CLIPResNetWithAttention(nn.Module): #1
    """
    A ResNet class that is similar to torchvision's but contains the following changes:
    - There are now 3 "stem" convolutions as opposed to 1, with an average pool instead of a max pool.
    - Performs anti-aliasing strided convolutions, where an avgpool is prepended to convolutions with stride > 1
    ...
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=None,
        ...
        # the 3-layer stem for alpha #改AlphaTCM4
        self.alpha_conv1 = nn.Conv2d(3, width // 2, kernel_size=3, stride=2, padding=1, bias=False)
        self.alpha_bn1 = nn.BatchNorm2d(width // 2)
        self.alpha_conv2 = nn.Conv2d(width // 2, width // 2, kernel_size=3, padding=1, bias=False)
        self.alpha_bn2 = nn.BatchNorm2d(width // 2)
        self.alpha_conv3 = nn.Conv2d(width // 2, width, kernel_size=3, padding=1, bias=False)
        self.alpha_bn3 = nn.BatchNorm2d(width)
        self.alpha_avgpool = nn.AvgPool2d(2)
        self.alpha_relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, alpha): #改AlphaTCM5
    # def forward(self, x):

    # _______________________________#改AlphaTCM7
        mask_transform = transforms.Compose([
            transforms.Normalize(0.5, 0.26)
        ])

        alpha = mask_transform(alpha)
        ...
        def alpha_stem(alpha): #改AlphaTCM6
            for conv, bn in [(self.alpha_conv1, self.alpha_bn1), (self.alpha_conv2, self.alpha_bn2), (self.alpha_conv3, self.alpha_bn3)]:
                alpha = self.alpha_relu(bn(conv(alpha)))
            alpha = self.alpha_avgpool(alpha)
            return alpha

        x = x.type(self.conv1.weight.dtype)
        x = stem(x)

    # _______________________________#改AlphaTCM8
        alpha = alpha.type(self.alpha_conv1.weight.dtype)
        alpha = alpha_stem(alpha)
    ...
    # _______________________________
        ...
        # x_global, x_local = self.attnpool(x) #改sclip
            ...
            for k in checkpoint.keys():
                ...
                if k == 'positional_embedding' or k == 'text_projection' or k.startswith(
                    ...
                    if k == 'positional_embedding' and checkpoint[k].size(0) > self.context_length:
                        ...
                        print_log(f'positional_embedding is tuncated from 77 to {self.context_length}',
                                  get_root_logger())
                    state_dict[k] = checkpoint[k]
...
@BACKBONES.register_module()
class CLIPTextContextEncoder(nn.Module): #2
    def __init__(self, context_length=22,
        ...
        # self.context_length = context_length #longclip改5
        self.context_length = 134
        ...
        self.mask1 = torch.zeros([134, 1])
        self.mask1[:20, :] = 1
        self.mask2 = torch.zeros([134, 1])
        self.mask2[20:, :] = 1
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            for k in checkpoint.keys():
                ...
                if k == 'positional_embedding' or k == 'text_projection' or k.startswith(
                        'token_embedding') or k.startswith('ln_final'):
                    if k == 'positional_embedding' and checkpoint[k].size(0) > self.context_length: #self.context_length=18
                        checkpoint[k] = checkpoint[k][:self.context_length]
                        print_log(f'positional_embedding is tuncated from 77 to {self.context_length}',
                                  get_root_logger())
                    state_dict[k] = checkpoint[k]
            # ————————————————————————————————————#longclip改4
            embedding_model = state_dict[k]
            positional_embedding_pre = state_dict[k].type(embedding_model.dtype)
            # dim = positional_embedding_pre.shape
            dim = 512
            length = 77
            keep_len = 20
            # posisitonal_embedding_new = torch.zeros([4 * length - 3 * keep_len, dim], dtype=embedding_model.dtype)
            posisitonal_embedding_new = torch.zeros([2 * length - keep_len, dim], dtype=embedding_model.dtype)
            for i in range(keep_len):
                posisitonal_embedding_new[i] = positional_embedding_pre[i]
                for i in range(length - 1 - keep_len):
                    posisitonal_embedding_new[2 * i + keep_len] = positional_embedding_pre[i + keep_len]
                    posisitonal_embedding_new[2 * i + 1 + keep_len] = positional_embedding_pre[i + keep_len] / 2 + 1 * \
                                                                      positional_embedding_pre[i + 1 + keep_len] / 2


                posisitonal_embedding_new[2 * length - keep_len - 2] = positional_embedding_pre[length - 1] + 0 * (
                        positional_embedding_pre[length - 1] - positional_embedding_pre[length - 2]) / 2
                posisitonal_embedding_new[2 * length - keep_len - 1] = positional_embedding_pre[length - 1] + 1 * (
                        positional_embedding_pre[length - 1] - positional_embedding_pre[length - 2]) / 2
            ...
            # state_dict['positional_embedding'] = nn.Parameter(posisitonal_embedding_new, requires_grad=False)
            # state_dict['positional_embedding_res'] = nn.Parameter(positional_embedding_res, requires_grad=True)
            # ————————————————————————————————————

            # u, w = self.load_state_dict(state_dict, False)#longclip改6
            # print_log(f'{u} {w} are misaligned params in text encoder', get_root_logger())
    ...
    def forward(self, text, context=None, use_learnable_prompt_only=False, prompt_gen=None):
        ...
        # x = x + self.positional_embedding #longclip改1
