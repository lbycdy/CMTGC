...
class MultiTaskProjector_V009(nn.Module):
    def __init__(self, word_dim=1024, in_dim=256, kernel_size=3):
        super().__init__()
        self.in_dim = in_dim
        self.kernel_size = kernel_size
...
class AdjustLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        #     x = x[:, :, l:r, l:r]
        return x
class MultiAdjustLayer(nn.Module):
    def __init__(self, in_channels, out_channels, weighted=False):
        super(MultiAdjustLayer, self).__init__()
    ...
    def forward(self, features):
        ...
        if self.weighted:
            return weighted_avg(out, weight)
        else:
            return avg(out)
...
class FPN_S8S16(nn.Module):
    def __init__(self,
                 in_channels=[512, 1024],
                 out_channels=[256, 512]):
        super(FPN_S8S16, self).__init__()
        # text projection
        self.txt_proj = linear_layer(in_channels[1], out_channels[1])
        ...
        self.f4_proj3 = conv_layer(out_channels[1], out_channels[1], 3, 1)
        # aggregation
        self.aggr = conv_layer(2 * out_channels[1], out_channels[1], 1, 0)
        self.coordconv = nn.Sequential(
            CoordConv(out_channels[1], out_channels[1], 3, 1),
            conv_layer(out_channels[1], out_channels[1], 3, 1))

    def forward(self, imgs, state):
        # v3, v4: 256, 52, 52 / 512, 26, 26
        v3, v4= imgs
        state = self.txt_proj(state).unsqueeze(-1).unsqueeze(-1)  # b, 1024, 1, 1
        f4 = self.f2_v_proj(v4)
        f4 = self.norm_layer(f4 * state)
        # fusion 3: b, 256, 26, 26
        f3 = self.f3_v_proj(v3)
        f3 = F.avg_pool2d(f3, 2, 2)
        f3 = self.f3_cat(torch.cat([f3, f4], dim=1))
        # fusion 4: b, 512, 13, 13 / b, 512, 26, 26
        fq4 = self.f4_proj4(f4)
        fq3 = self.f4_proj3(f3)
        # query
        fq = torch.cat([fq3, fq4], dim=1)
        fq = self.aggr(fq)
        fq = self.coordconv(fq)
        # b, 512, 26, 26
        return fq
