...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.attnpool8 = AttentionPool2d(input_resolution // 32, embed_dim // 4, heads, output_dim//2)
    ...
    def forward(self, x, mask = None):
        ...
        x = self.layer1(x)
        x2 = self.layer2(x)
        x2_temp = self.attnpool8(x2)
        x3 = self.layer3(x2)
        x3_temp = self.attnpool16(x3)
        x4 = self.layer4(x3)
        x4_temp = self.attnpool(x4)
        return (x2_temp, x3_temp, x4_temp)
...
def build_model(state_dict: dict, txt_length: int, load_weights=True, load_twin_weights = False):
    ...
    if load_weights:
        ...
        #     model.visual.conv1_mask.weight = model.visual.conv1.weight
    return model.eval()
...
class CROG_TWIN(nn.Module):
    ...
    def forward(self, img,image_swin, word, mask=None, grasp_qua_mask=None, grasp_sin_mask=None, grasp_cos_mask=None,
        ...
        # word: b, length, 1024
        # state: b, 1024
        vis = self.backbone.encode_image(img, mask)
