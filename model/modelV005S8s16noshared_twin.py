...
from .layers import FPN_S8S16, Projector, TransformerDecoder, MultiTaskProjector
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self._inplanes_twin = width
        self.layer1 = self._make_layer(width, layers[0])
        self.layer2 = self._make_layer(width * 2, layers[1], stride=2)
        self.layer3 = self._make_layer(width * 4, layers[2], stride=2)
        self.layer4 = self._make_layer(width * 8, layers[3], stride=2)

        self.layer1_twin = self._make_layer_twin(width, layers[0])
        self.layer2_twin = self._make_layer_twin(width * 2, layers[1], stride=2)
        self.layer3_twin = self._make_layer_twin(width * 4, layers[2], stride=2)
        self.layer4_twin = self._make_layer_twin(width * 8, layers[3], stride=2)

        embed_dim = width * 32  # the ResNet feature dimension
        self.attnpool = AttentionPool2d(input_resolution // 32, embed_dim, heads, output_dim)
        self.attnpool16 = AttentionPool2d(input_resolution // 32, embed_dim // 2, heads, output_dim)
    ...
    def _make_layer_twin(self, planes, blocks, stride=1):
        layers = [Bottleneck_Twin(self._inplanes_twin, planes, stride)]

        self._inplanes_twin = planes * Bottleneck_Twin.expansion
        for _ in range(1, blocks):
            layers.append(Bottleneck_Twin(self._inplanes_twin, planes))
    ...
    def forward(self, x, image_swin = None):
        ...
        x = stem(x)
        x_twin = image_swin.type(self.conv1.weight.dtype)
        x_twin = stem_twin(x_twin)

        x = self.layer1(x)
        x_twin = self.layer1_twin(x_twin)
        x2 = self.layer2(x)
        x_twin2 = self.layer2_twin(x_twin)
        x2 = x2 + x_twin2
        x3 = self.layer3(x2)
        x_twin3 = self.layer3_twin(x_twin2)
        x3 = x3 + x_twin3
        x3 = self.attnpool16(x3)
        return (x2, x3)
...
def build_model(state_dict: dict, txt_length: int, load_weights=True, load_twin_weights = False):
    ...
    if load_weights:
        ...
        if load_twin_weights:
            model.visual.conv1_twin = model.visual.conv1
            model.visual.bn1_twin = model.visual.bn1
            model.visual.conv2_twin = model.visual.conv2
            model.visual.bn2_twin = model.visual.bn2
            model.visual.conv3_twin = model.visual.conv3
            model.visual.bn3_twin = model.visual.bn3

            model.visual.layer1_twin = model.visual.layer1
            model.visual.layer2_twin = model.visual.layer2
            model.visual.layer3_twin = model.visual.layer3
...
class CROG_TWIN(nn.Module):
    def __init__(self, cfg):
        ...
        # Multi-Modal FPN
        self.neck = FPN_S8S16(in_channels=cfg.fpn_in, out_channels=cfg.fpn_out)
