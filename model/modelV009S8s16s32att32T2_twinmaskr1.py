...
from .layers import FPN, FPN_S8S16, Projector, TransformerDecoder, MultiTaskProjector_V009
...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, mask = None, x_twin=None):
        def stem(x, mask):
        ...
        mask = mask.type(self.conv1.weight.dtype)
        x = x.type(self.conv1.weight.dtype)
        ...
        x_twin1 = self.layer1(x_twin)
        x_twin2 = self.layer2(x_twin1)
        x_twin3 = self.layer3(x_twin2)
        x_twin4 = self.layer4(x_twin3)
        x_twin4 = self.attnpool(x_twin4)


        return (x2, x3, x4), (x_twin2, x_twin3, x_twin4)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, mask, image_twin):
        assert mask is not None
        return self.visual(image.type(self.dtype), mask.type(self.dtype), image_twin)
    ...
    def forward(self, image,image_twin, mask, text):
        image_features, image_twin_features = self.encode_image(image, mask, image_twin)
        text_features = self.encode_text(text)

        # normalized features
        image_features = image_features / image_features.norm(dim=-1,
                                                              keepdim=True)
        image_twin_features = image_twin_features / image_twin_features.norm(dim=-1,
                                                              keepdim=True)
        ...
        image_twin_features = logit_scale * image_twin_features @ text_features.t()
        logits_per_text = image_twin_features.t()

        # shape = [global_batch_size, global_batch_size]
        return logits_per_image,image_twin_features, logits_per_text
...
class CROG_TWIN(nn.Module):
    def __init__(self, cfg):
        ...
        if self.use_grasp_masks:
            ...
            self.proj = MultiTaskProjector_V009(cfg.word_dim, cfg.vis_dim // 2, 3)
        else:
    ...
    def forward(self, img, image_swin, word, mask=None, grasp_qua_mask=None, grasp_sin_mask=None, grasp_cos_mask=None,
        ...
        # word: b, length, 1024
        # state: b, 1024
        vis, vis_twin = self.backbone.encode_image(img, mask, image_swin)

        word, state = self.backbone.encode_text(word)

        # b, 512, 26, 26 (C4)
        fq = self.neck(vis_twin, state)
        b, c, h, w = fq.size()
        ...
        vis1 = vis[1]
        vis2 = F.interpolate(vis[2], scale_factor=2, mode='bilinear')
        fq = torch.cat([vis1, vis2, fq], dim=1)
