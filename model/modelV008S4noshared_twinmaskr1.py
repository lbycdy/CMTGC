...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x, mask = None, img_twin = None, masktype = None):
        def stem(x):
            ...
            x = self.avgpool(x)
            return x
        def stem_twin(x_twin):
            if masktype==1:
                x = self.relu_twin(self.bn1_twin(self.conv1_twin(x_twin)))
            elif masktype==2:
                x = self.relu_twin(self.bn1_twin(self.conv1_twin(x_twin)+self.conv1_mask(mask)))
        ...
        x = stem(x)
        x_twin = img_twin.type(self.conv1.weight.dtype)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, mask, image_twin, mask_type):
        assert mask is not None
        assert image_twin is not None
        return self.visual(image.type(self.dtype), mask.type(self.dtype), image_twin.type(self.dtype), mask_type)
    ...
    def forward(self, image, mask, image_twin, text, mask_type):
        image_features = self.encode_image(image, mask, image_twin, mask_type)
...
class CROG_TWIN(nn.Module):
    ...
    def forward(self, img,image_swin, word, mask=None, grasp_qua_mask=None, grasp_sin_mask=None, grasp_cos_mask=None,
        ...
        # word: b, length, 1024
        # state: b, 1024
        vis = self.backbone.encode_image(img, mask, image_swin, mask_type=self.mask_type)
