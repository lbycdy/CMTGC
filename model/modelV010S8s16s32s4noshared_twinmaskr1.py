...
from .layers import FPN, FPN_S8S16, Projector, TransformerDecoder, MultiTaskProjector
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        out = self.relu(out)
        return out
class Bottleneck_Twin(nn.Module):
    expansion = 4
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        # all conv layers have stride 1. an avgpool is performed after the second convolution when stride > 1
        self.conv1_twin = nn.Conv2d(inplanes, planes, 1, bias=False)
        self.bn1_twin = nn.BatchNorm2d(planes)

        self.conv2_twin = nn.Conv2d(planes, planes, 3, padding=1, bias=False)
        self.bn2_twin = nn.BatchNorm2d(planes)

        self.avgpool_twin = nn.AvgPool2d(stride) if stride > 1 else nn.Identity()

        self.conv3_twin = nn.Conv2d(planes, planes * self.expansion, 1, bias=False)
        self.bn3_twin = nn.BatchNorm2d(planes * self.expansion)

        self.relu_twin = nn.ReLU(inplace=True)
        self.downsample_twin = None
        self.stride = stride

        if stride > 1 or inplanes != planes * Bottleneck.expansion:
            # downsampling layer is prepended with an avgpool, and the subsequent convolution has stride 1
            self.downsample_twin = nn.Sequential(
                OrderedDict([("-1", nn.AvgPool2d(stride)),
                             ("0",
                              nn.Conv2d(inplanes,
                                        planes * self.expansion,
    ...
    def forward(self, x: torch.Tensor):
        identity = x
        out = self.relu_twin(self.bn1_twin(self.conv1_twin(x)))
        out = self.relu_twin(self.bn2_twin(self.conv2_twin(out)))
        out = self.avgpool_twin(out)
        out = self.bn3_twin(self.conv3_twin(out))

        if self.downsample_twin is not None:
            identity = self.downsample_twin(x)

        out += identity
        out = self.relu_twin(out)
        return out
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(3,
                               ...
                               padding=1,
                               bias=False)
        self.conv1_mask = nn.Conv2d(in_channels=1, out_channels=width // 2, kernel_size=3, stride=2, padding=1, bias=False)
        self.conv1_twin = nn.Conv2d(3, width // 2, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        self.conv2 = nn.Conv2d(width // 2,
                               ...
                               padding=1,
                               bias=False)
        self.conv2_twin = nn.Conv2d(width // 2, width // 2, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x, mask = None, image_swin = None):
        def stem(x):

            x = self.relu(self.bn1(self.conv1(x) + self.conv1_mask(mask)))
            x = self.relu(self.bn2(self.conv2(x)))
            x = self.relu(self.bn3(self.conv3(x)))
            x = self.avgpool(x)
            return x
        def stem_twin(x):
            x = self.relu_twin(self.bn1_twin(self.conv1_twin(x)))
            x = self.relu_twin(self.bn2_twin(self.conv2_twin(x)))
            x = self.relu_twin(self.bn3_twin(self.conv3_twin(x)))
            x = self.avgpool_twin(x)
            return x
        x = x.type(self.conv1.weight.dtype)
        image_swin = image_swin.type(self.conv1.weight.dtype)
        x = stem(x)
        x_twin = stem_twin(image_swin)
        x = x + x_twin
        x = self.layer1(x)
        x2 = self.layer2(x)
        x3 = self.layer3(x2)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, mask, image_swin):
        assert mask is not None
        return self.visual(image.type(self.dtype), mask.type(self.dtype), image_swin.type(self.dtype))
...
class CROG_TWIN(nn.Module):
    ...
    def forward(self, img,image_swin, word, mask=None,mask_all=None, grasp_qua_mask=None, grasp_sin_mask=None, grasp_cos_mask=None,
                grasp_wid_mask=None):
        '''
            img: b, 3, h, w
            word: b, words
        ...
        # word: b, length, 1024
        # state: b, 1024
        vis = self.backbone.encode_image(img, mask, image_swin)
        ...
        import cv2
        # binary_mask_sample = mask_all[0,0].cpu().numpy()  # 转换为 NumPy 数组，形状为 (416, 416)
        # cv2.imshow('Binary Mask', binary_mask_sample)
        # cv2.waitKey(0)  # 按任意键关闭窗口
        # cv2.destroyAllWindows()

        # feature_map = fq.detach().cpu().numpy()
        ...
        #
        # original_image = img[0].detach().cpu().numpy()  # 转换为 numpy 数组
        ...
        # 1. 调整 mask 的大小以匹配 fq 的空间维度 (26, 26)
        mask_resized = F.interpolate(mask_all, size=(26, 26), mode='bilinear',
                                     align_corners=False)  # (12, 1, 26, 26)
        # 2. 将 mask 扩展到与 fq 匹配的通道数
        mask_resized = mask_resized.expand(-1, fq.size(1), -1, -1)  # (12, 512, 26, 26)

        mask_resized = 0.5 + 0.5 * mask_resized
        # max_value = torch.max(mask_resized)
        # min_value = torch.min(mask_resized)
        # print(f"Max value: {max_value}")
        # print(f"Min value: {min_value}")

        # 逐元素相乘
        result = fq * mask_resized

        if self.use_grasp_masks:

            # b, 1, 104, 104
            pred, grasp_qua_pred, grasp_sin_pred, grasp_cos_pred, grasp_wid_pred = self.proj(result, state)
...
if __name__ == '__main__':
    ...
    def get_parser():
        ...
        parser.add_argument('--config', default='/home/lbycdy/work/crog_lb/config/OCID-VLG/test.yaml', type=str, help='config file')
        parser.add_argument('--opts', default=None, nargs=argparse.REMAINDER, help='override some settings in the config.')
        parser.add_argument('--image_path', default='ARID20/table/bottom/seq11/rgb/result_2018-08-21-16-32-13.png', type=str, required=True, help='Path to the input image')
        parser.add_argument('--mask_path',default='ARID20/table/bottom/seq11/seg_mask_instances_combi/result_2018-08-21-16-32-13.png', type=str, required=True, help='Path to the input mask')
        parser.add_argument('--instruction', default='grasp the marker', required=True, help='Instruction for the model')
    ...
    img = torch.tensor(img).permute(2, 0, 1).unsqueeze(0).float()  # Convert to tensor and add batch dimension
    mask = torch.tensor(mask).unsqueeze(0).unsqueeze(0).float()  # Convert to tensor and add batch dimension and channel dimension

    # Tokenize instruction
    tokenizer = _Tokenizer()
    word = tokenizer.encode(args.instruction)
    word = torch.tensor(word).unsqueeze(0)  # Add batch dimension

    # Ensure word length matches attn_mask shape
    txt_length = word.size(1)
    cfg.word_len = txt_length
    ...
    image_swin = img.clone()  # Assuming image_swin is similar to img
    mask_all = mask.clone()  # Assuming mask_all is similar to mask
    grasp_qua_mask = torch.zeros_like(mask)  # Initialize grasp quality mask
    grasp_sin_mask = torch.zeros_like(mask)  # Initialize grasp sine mask
    grasp_cos_mask = torch.zeros_like(mask)  # Initialize grasp cosine mask
    grasp_wid_mask = torch.zeros_like(mask)  # Initialize grasp width mask

    # Forward pass
    with torch.no_grad():
        output = model(img, image_swin, word, mask, mask_all, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask, grasp_wid_mask)

    # Process output
    pred = output[0][0].squeeze().cpu().numpy()
    cv2.imshow('Prediction', pred)
    cv2.waitKey(0)
    cv2.destroyAllWindows()


#python model/modelV010S8s16s32s4noshared_twinmaskr1.py  --image_path '/home/lbycdy/work/datasets/OCID-VLG/ARID20/table/bottom/seq11/rgb/result_2018-08-21-16-32-13.png' --mask_path '/home/lbycdy/work/datasets/OCID-VLG/ARID20/table/bottom/seq11/seg_mask_instances_combi/result_2018-08-21-16-32-13.png'  --instruction 'grasp the marker'
